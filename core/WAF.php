...
class WAF {
    ...
    public static function hataliGirisKaydet($ip) {
        $db = Database::baglan();
        // Son 5 dakikadaki hatalı giriş sayısını kontrol et
    ...
    private static function otonomBanla($ip, $sebep) {
        $db = Database::baglan();
        ...
        Mail::gonder($adminMail, "⚠️ [GÜVENLİK ALARMI] IP Otonom Engellendi", "
            <div style='font-family:sans-serif; background:#0f172a; color:#fff; padding:20px; border-radius:10px;'>
                ...
                <p>Sistem insan müdahalesine gerek kalmadan IP adresini karalisteye almıştır.</p>
            </div>
        ");

        http_response_code(403);
        die("<!DOCTYPE html><html><head><title>WAF Blocked</title></head><body style='background:#0f172a;color:#f43f5e;font-family:sans-serif;padding:50px;text-align:center;'><h1>🛡️ Otonom WAF Kalkanı Devrede</h1><p>Zararlı istek parametresi algılandı. IP adresiniz ({$ip}) engellendi.</p></body></html>");
    }
}
